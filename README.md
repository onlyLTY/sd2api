...
| `feishu_receive_id_type` / `feishu_receive_id` | `chat_id` / - | 通知对象；在后台直接从群聊或通讯录下拉列表选择，无需手工填写 ID |
...
在飞书开放平台创建企业自建应用并启用机器人，为应用开通发送消息、读取群信息和读取通讯录成员权限，同时设置通讯录可用范围。群通知还需要先把机器人加入目标群。随后在“系统配置 > 飞书通知”中保存 App ID 和 App Secret，选择个人或群聊，程序会按应用实际获准访问的通讯录范围读取联系人/群列表，无需开放整个组织的根部门；选中目标后可直接点击“发送飞书测试消息”。

自动通知覆盖图形验证码、需要手动输入邮件验证码、登录失败、Chromium 异常、凭据未配置和 session 失效需重新登录。相同账号在同一次异常期间只通知一次；状态恢复后再次发生异常时会重新通知。
