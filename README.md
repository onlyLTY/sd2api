...
在飞书开放平台创建企业自建应用并启用机器人，为应用开通发送消息、读取群信息和读取通讯录成员权限，同时设置通讯录可用范围。群通知还需要先把机器人加入目标群。随后在“系统配置 > 飞书通知”中保存 App ID 和 App Secret，选择个人或群聊，程序会读取可用联系人/群列表；选中目标后可直接点击“发送飞书测试消息”。
